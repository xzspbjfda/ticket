...
export default {
	...
	platformGroupFilter() {
		let platformGroup = 'maiKe';
		// #ifdef H5
		if ($mPayment.isWechat()) {
			platformGroup = 'maiKeWechat';
		} else {
			platformGroup = 'maiKeH5';
		}
		// #endif
		// #ifdef MP-QQ
		platformGroup = 'maiKeQqMp';
		// #endif
		// #ifdef MP-WEIXIN
		platformGroup = 'maiKeWechatMp';
		// #endif
		// #ifdef MP-ALIPAY
		platformGroup = 'maiKeAliMp';
		// #endif
		// #ifdef MP-QQ
		platformGroup = 'maiKeQqMp';
		// #endif
		// #ifdef MP-BAIDU
		platformGroup = 'maiKeBaiduMp';
		// #endif
		// #ifdef APP-PLUS
		switch (uni.getSystemInfoSync().platform) {
			case 'android':
				platformGroup = 'maiKeAndroid';
				break;
			case 'ios':
				platformGroup = 'maiKeIos';
				break;
		}
		// #endif
	...
	handleBannerNavTo(data, id, advId) {
		...
		switch (data) {
			...
			case 'product_list_for_cate': // 某分类下产品列表
				...
				break;
			case 'big_wheel_view': // 大转盘
				url = `/pages/marketing/activity/lucky-wheel?id=${id}`;
				break;
	...
	// 自定义装修通用跳转封装
	handleBasicNavTo(type, route, id, urlType, innerLink, externalLink) {
		if (!type) {
			if (
				route.indexOf('/pages/profile/profile') !== -1 ||
				route.indexOf('/pages/cart/cart') !== -1 ||
				route.indexOf('/pages/index/index') !== -1 ||
				route.indexOf('/pages/notify/notify') !== -1 ||
				route.indexOf('/pages/category/category') !== -1
			) {
				mRouter.switchTab({ route });
			} else {
				mRouter.push({ route });
			}
		} else {
			...
			switch (type) {
				...
				case 'hot':
					...
					break;
				case 'custom':
					url = `/pages/index/activity?id=${id}`;
					break;
				case 'bigWheel':
					url = `/pages/marketing/activity/lucky-wheel?id=${id}`;
					break;
				case 'externalLink':
					url = `/pages/public/url?url=${externalLink}`;
					break;
				case 'innerLink':
					if (innerLink.indexOf('/pages/profile/profile') !== -1 ||
						innerLink.indexOf('/pages/cart/cart') !== -1 ||
						innerLink.indexOf('/pages/index/index') !== -1 ||
						innerLink.indexOf('/pages/notify/notify') !== -1 ||
						innerLink.indexOf('/pages/category/category') !== -1
					) {
						mRouter.switchTab({ route: innerLink });
					} else {
						mRouter.push({ route: innerLink });
					}
					return;
				default:
					url = `/pages/product/product?id=${id}`;
