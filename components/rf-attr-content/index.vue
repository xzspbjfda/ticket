...
<!--				></rf-number-box>-->
		...
		<button v-if="!showBuyBtn" class="btn" :class="'bg-green'" @tap="toggleSpec">完成</button>
		<view class="btn-group" v-else>
			<button class="btn" :class="'bg-green'" @tap="toggleSpec(1)">加入购物车</button>
			<button class="btn" :class="'bg-green' " @tap="toggleSpec(2)">立即购买</button>
		</view>
	</view>
</template>
<script>
...
export default {
  ...
  methods: {
    initData() {
			...
			this.specList && this.specList.forEach(item => {
		...
		selectSpec(index, pid, type) {
			...
			this.product.sku.forEach(item => {
				if (item.data === skuStr.join('-')) {
					...
					this.stock = item.stock;
					this.price = this.type === 'wholesale' ? item.wholesale_price : item.price;
					this.skuId = item.id;
					this.skuName = item.name;
				}
				// else {
