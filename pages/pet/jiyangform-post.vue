	...
	<view class="wrap">
		<view class="subwrap">
			<u-form ref="uForm" :model="item" :rules="rules" label-position="top" :border-bottom="false" :errorType="errorType">
				...
				<block >
					<u-form-item :label=" '预约时间'" prop="datetime_name" :border-bottom="false" :label-style="labelStyle">
						<u-input type="select" v-model="item.advance_time" height="100" disabled :custom-style="customStyle" placeholder="请选择" @click="datetimeShow = true" />
					</u-form-item>
				</block>
				
				<u-gap height="20" />
				<u-form-item label="寄养天数" prop="day" :border-bottom="false" :label-style="labelStyle">
					<u-input v-model="item.day" height="100" :custom-style="customStyle" trim placeholder="请输入" />
				</u-form-item>
				...
				<u-form-item label="自带粮食" prop="food" label-position="left" :border-bottom="false" label-width="auto" :label-style="labelStyle"></u-form-item>
				<u-subsection :list="$mData.liangshi" :current="item.food" mode="button" height="68" :bold="false" active-color="#FFF" :button-color="themeColor.color" @change="liangshiSectionChange" />
				...
				<u-form-item label="寄养人联系方式" :border-bottom="false" :label-style="labelStyle"></u-form-item>
				
				<view class="contact">
					<u-form-item label="微信号" prop="send_name" label-position="left" :border-bottom="false" label-width="auto" :label-style="labelStyle02">
						<u-input v-model="item.send_name" height="50" input-align="right" :custom-style="customStyle03" trim placeholder="请输入" />
					</u-form-item>
					
					<u-form-item label="手机号" prop="send_tel" label-position="left" :border-bottom="false" label-width="auto" :label-style="labelStyle02">
						<u-input v-model="item.send_tel" height="50" input-align="right" :custom-style="customStyle03" trim placeholder="请输入" />
					</u-form-item>
...
<script>
import { jiyangMyView, jiyangForm, fileImages, fileVideos } from '@/api/pet';
...
export default {
	...
	methods: {
		...
		async getPetMyView() {
			await this.$http.get(`${jiyangMyView}`, {
				id: this.id
			}).then(async r => {
				this.loading = false;
				this.item = r.data;
