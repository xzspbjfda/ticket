	...
	<view class="wrap">
		<view class="subwrap">
			<u-form ref="uForm" :model="item" :rules="rules" label-position="top" :border-bottom="false" :errorType="errorType">
				<u-form-item label="留言内容" prop="content" :border-bottom="false" :label-style="labelStyle">
...
<script>
import { commentView, commentForm } from '@/api/pet';
...
export default {
	...
	methods: {
		...
		async getCommentView() {
			await this.$http.get(`${commentView}?expand=member`, {
				id: this.id
			}).then(async r => {
				this.loading = false;
				this.item = r.data;
