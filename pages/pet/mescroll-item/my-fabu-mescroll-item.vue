<template>
	 <mescroll-uni :ref="'mescrollRef' + i" height="100%" top="0" @init="mescrollInit" :up="upOption" @up="upCallback" :down="downOption" @down="downCallback" @emptyclick="emptyClick">
		<view class="item" v-for="(item, index) in items" :key="index" @tap="navTo(`/pages/product/product?id=${item.id}`)">
			<view class="left"><u-image :src="item.picture" width="100%" height="210" border-radius="20" mode="aspectFill" /></view>
			...
			<view class="right">
				...
				<view class="base">
					<view class="tag">{{item.price}}</view>
			...
			<view class="bottom">
				<view class="meta">
					...
					<view class="share">销量{{item.total_sales}}</view>
				</view>
				
				<u-line margin="20rpx 0 0 0" />
				
				<view class="option">
					<u-button type="primary" size="mini" shape="circle" plain @click="navTo(`/pages/pet/fabu-post?id=${item.id}`)">编辑</u-button>
...
import { fabuList, petStateOn, petStateOff, fabuTrash } from '@/api/pet';
...
export default {
	...
	methods: {
		...
		async upCallback(page) {
			...
			await this.$http.get(`${fabuList}`, params).then(async r => {
