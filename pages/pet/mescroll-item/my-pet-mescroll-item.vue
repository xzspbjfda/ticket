<template>
	 <mescroll-uni :ref="'mescrollRef' + i" height="100%" top="0" @init="mescrollInit" :up="upOption" @up="upCallback" :down="downOption" @down="downCallback" @emptyclick="emptyClick">
		<view class="item" v-for="(item, index) in items" :key="index" @tap="navTo(`/pages/pet/pet-view?id=${item.id}`)">
			<view class="left"><u-image :src="item.poster" width="100%" height="210" border-radius="20" mode="aspectFill" /></view>
			
			<view class="right">
				<view class="title u-line-1">
					[{{item.state_name}}]{{item.title}}
				</view>
			...
			<view class="bottom">
				...
				<view class="option">
					<u-button type="primary" size="mini" shape="circle" plain @click="navTo(`/pages/pet/pet-post?id=${item.id}`)">编辑领养</u-button>
					<u-button type="warning" size="mini" shape="circle" plain @click="petStateOn(item.id)" v-if="item.state == 0">设为已完成</u-button>
					<u-button type="warning" size="mini" shape="circle" plain @click="petStateOff(item.id)" v-if="item.state == 1">设为进行中</u-button>
					<u-button type="error" size="mini" shape="circle" plain @click="petTrash(item.id)">删除</u-button>
				</view>
			</view>
		</view>
	</mescroll-uni>
...
import { petMyList, petStateOn, petStateOff, petTrash } from '@/api/pet';
...
export default {
	...
	methods: {
		...
		async upCallback(page) {
			...
			await this.$http.get(`${petMyList}`, params).then(async r => {
