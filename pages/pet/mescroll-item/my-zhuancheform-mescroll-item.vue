		...
		<view class="item" v-for="(item, index) in items" :key="index" >
			...
			<view class="right">
				<view class="title u-line">
					开始地址:{{item.begin_address}}
				</view>
				<view class="title u-line">
					目的地地址:{{item.end_address}}
				</view>
				<view class="base">
					<view class="tag"v-if="item.type == 1">猫</view>
					<view class="tag"v-if="item.type == 2">狗</view>
					<view class="tag"v-if="item.type == 3">其他</view>
					<view class="tag"v-if="item.cage == 0">无笼子</view>
					<view class="tag"v-if="item.cage == 1">有笼子</view>
			...
			<view class="bottom">
				<view class="meta">
					<view class="datetime">出发时间：{{item.send_time}}</view>
				...
				<view class="option">
					<u-button type="primary" size="mini" shape="circle" plain @click="navTo(`/pages/pet/zhuancheform-post?id=${item.id}`)">编辑查看</u-button>

					<u-button type="error" size="mini" shape="circle" plain @click="zhuancheTrash(item.id)">删除</u-button>
				</view>
			</view>
		</view>
	</mescroll-uni>
...
import { zhuancheformMyList, petStateOn, petStateOff, zhuancheTrash } from '@/api/pet';
...
export default {
	...
	methods: {
		...
		async upCallback(page) {
			...
			await this.$http.get(`${zhuancheformMyList}`, params).then(async r => {
