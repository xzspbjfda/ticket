		...
		<view class="item" v-for="(item, index) in items" :key="index" @tap="navTo(`/pages/pet/forum-view?id=${item.id}`)">
			...
			<view class="bottom">
				...
				<view class="option">
					...
					<u-button type="primary" size="mini" shape="circle" plain @click="postStateOn(item.id)" v-if="item.state == 0">设为显示</u-button>
					<u-button type="primary" size="mini" shape="circle" plain @click="postStateOff(item.id)" v-if="item.state == 1">设为隐藏</u-button>
