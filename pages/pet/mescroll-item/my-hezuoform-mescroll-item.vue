		...
		<view class="item" v-for="(item, index) in items" :key="index" >
			...
			<view class="right">
				...
				<view class="base">
					...
					<view class="tag">类型:{{item.selltypes}}</view>
				</view>
			...
			<view class="bottom">
				...
				<view class="option">
					<u-button type="primary" size="mini" shape="circle" plain @click="navTo(`/pages/pet/hezuoform-post?id=${item.id}`)">编辑查看</u-button>

					<u-button type="error" size="mini" shape="circle" plain @click="hezuoTrash(item.id)">删除</u-button>
				</view>
			</view>
		</view>
	</mescroll-uni>
...
import { hezuoformMyList, petStateOn, petStateOff, hezuoTrash } from '@/api/pet';
...
export default {
	...
	methods: {
		...
		async upCallback(page) {
			...
			await this.$http.get(`${hezuoformMyList}`, params).then(async r => {
