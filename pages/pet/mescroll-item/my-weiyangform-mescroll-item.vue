	 ...
	 <mescroll-uni :ref="'mescrollRef' + i" height="100%" top="0" @init="mescrollInit" :up="upOption" @up="upCallback" :down="downOption" @down="downCallback" @emptyclick="emptyClick">
		<view class="item" v-for="(item, index) in items" :key="index" >
			...
			<view class="bottom">
				...
				<view class="option">
					<u-button type="primary" size="mini" shape="circle" plain @click="navTo(`/pages/pet/weiyangform-post?id=${item.id}`)">编辑查看</u-button>

					<u-button type="error" size="mini" shape="circle" plain @click="weiyangTrash(item.id)">删除</u-button>
				</view>
			</view>
		</view>
	</mescroll-uni>
...
import { weiyangformMyList, petStateOn, petStateOff, weiyangTrash } from '@/api/pet';
...
export default {
	...
	methods: {
		...
		async upCallback(page) {
			...
			await this.$http.get(`${weiyangformMyList}`, params).then(async r => {
