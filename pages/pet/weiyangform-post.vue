	...
	<view class="wrap">
		<view class="subwrap">
			<u-form ref="uForm" :model="item" :rules="rules" label-position="top" :border-bottom="false" :errorType="errorType">
				...
				<u-gap height="20" />
				<u-form-item label="服务天数" prop="day" :border-bottom="false" :label-style="labelStyle">
					<u-input v-model="item.day" height="100" :custom-style="customStyle" trim placeholder="请输入" />
				</u-form-item>
				<u-gap height="20" />
				<u-form-item label="服务报酬" prop="bao_chou" :border-bottom="false" :label-style="labelStyle">
					<u-input v-model="item.bao_chou" height="100" :custom-style="customStyle" trim placeholder="请输入" />
				</u-form-item>
				...
				<u-form-item label="送养人联系方式" :border-bottom="false" :label-style="labelStyle"></u-form-item>
				
				<view class="contact">
					<u-form-item label="微信号码" prop="wx_number" label-position="left" :border-bottom="false" label-width="auto" :label-style="labelStyle02">
						<u-input v-model="item.wx_number" height="50" input-align="right" :custom-style="customStyle03" trim placeholder="请输入" />
					</u-form-item>
...
<script>
import { weiyangMyView, weiyangForm, fileImages, fileVideos } from '@/api/pet';
...
export default {
	...
	methods: {
		...
		async getPetMyView() {
			await this.$http.get(`${weiyangMyView}`, {
				id: this.id
			}).then(async r => {
				this.loading = false;
				this.item = r.data;
