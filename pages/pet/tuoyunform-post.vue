	...
	<view class="wrap">
		<view class="subwrap">
			<u-form ref="uForm" :model="item" :rules="rules" label-position="top" :border-bottom="false" :errorType="errorType">
				...
				<view class="contact">
					<u-form-item label="微信号" prop="begin_name" label-position="left" :border-bottom="false" label-width="auto" :label-style="labelStyle02">
						<u-input v-model="item.begin_name" height="50" input-align="right" :custom-style="customStyle03" trim placeholder="请输入" />
					</u-form-item>
...
<script>
import { tuoyunMyView, tuoyunForm, fileImages, fileVideos } from '@/api/pet';
...
export default {
	...
	methods: {
		...
		async getPetMyView() {
			await this.$http.get(`${tuoyunMyView}`, {
				id: this.id
			}).then(async r => {
				this.loading = false;
				this.item = r.data;
