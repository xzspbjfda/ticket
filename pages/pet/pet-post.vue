		...
		<view class="subwrap">
			<u-form ref="uForm" :model="item" :rules="rules" label-position="top" :border-bottom="false" :errorType="errorType">
				...
				<view class="contact">
					...
					<u-form-item label="手机号" prop="member.mobile" label-position="left" :border-bottom="false" label-width="auto" :label-style="labelStyle02">
						<u-input v-model="item.member.mobile" height="50" input-align="right" :custom-style="customStyle03" trim placeholder="请输入（选填）" />
					</u-form-item>
				</view>
	...
	methods: {
		async initLogin() {
			...
			if (this.hasLogin && this.userInfo) {
				...
				this.item.member.mobile = this.userInfo.mobile;
			} else {
