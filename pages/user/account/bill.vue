			...
			>
				...
				<view class="right">
					<text
						...
						"
						><text v-if="item.type == 2">+</text><text v-if="item.type == 1">-</text> {{ item.num | numFilter }}
					</text>
				</view>
			</view>
			<rf-load-more
		...
		async getIntegralListList(type) {
			...
			await this.$http
				...
				.then(r => {
					...
					this.loadingType = r.data.list.length < 10 ? 'nomore' : 'more';
					this.integralList = [...this.integralList, ...r.data.list];
				})
				.catch(() => {
					this.loading = false;
					if (type === 'refresh') {
...
.account-bill {
	.rf-bill-list {
		.rf-list-item {
			...
			border: 1rpx solid rgba(0, 0, 0, .02);
			display: flex;
